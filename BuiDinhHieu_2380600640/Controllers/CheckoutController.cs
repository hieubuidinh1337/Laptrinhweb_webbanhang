...
using BuiDinhHieu_2380600640.Services;
using System.Linq;
...
namespace BuiDinhHieu_2380600640.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public async Task<IActionResult> PlaceOrder(CheckoutViewModel vm)
        {
            ...
            if (vm.PaymentMethod?.ToLower() == "vnpay")
            {
                ...
                var vnpayRequest = new VNPayPaymentRequest
                {
                    ...
                    OrderInfo = $"Thanh toan don hang {order.OrderCode ?? orderId.ToString()} tai DinhHieu Atelier",
                    IpAddress = ipAddress,
                    Locale = "vn"
                };
        ...
        public async Task<IActionResult> Confirmation([FromQuery] int orderId) 
        {
            ...
            if (order == null)
            {
                return Content($"LỖI HỆ THỐNG: Không tìm thấy đơn hàng nào có ID là {orderId} trong Database! Hãy kiểm tra lại bảng [dbo].[Orders].");
            }
