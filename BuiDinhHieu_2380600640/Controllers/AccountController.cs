...
using BuiDinhHieu_2380600640.Models.ViewModels;
...
namespace BuiDinhHieu_2380600640.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel vm, string? returnUrl)
        {
            ...
            Microsoft.AspNetCore.Identity.SignInResult result = await _signInManager.PasswordSignInAsync(vm.Email, vm.Password, vm.RememberMe, lockoutOnFailure: true);
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel vm)
        {
            ...
            // Tạo Token xác nhận đặt lại mật khẩu từ Identity
            var token = await _userManager.GeneratePasswordResetTokenAsync(user);

            // Chuyển hướng thẳng sang trang đặt lại mật khẩu mới (Reset Password)
            // Truyền theo Email và Token qua URL để trang sau xử lý
            return RedirectToAction(nameof(ResetPassword), new { email = vm.Email, token = token });
        }

        [HttpGet]
        [AllowAnonymous]
        [Route("tai-khoan/dat-lai-mat-khau")]
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel vm)
        {
            ...
            if (user == null)
            {
                // Không tiết lộ user không tồn tại vì lý do bảo mật, chuyển về trang chủ
                return RedirectToAction("Index", "Home");
            }

            // Thực hiện đổi mật khẩu bằng Token thông qua UserManager
            var result = await _userManager.ResetPasswordAsync(user, vm.Token, vm.NewPassword);

            if (result.Succeeded)
            {
                TempData["Success"] = "Mật khẩu của bạn đã được đặt lại thành công. Vui lòng đăng nhập lại.";
                return RedirectToAction(nameof(Login));
            }
